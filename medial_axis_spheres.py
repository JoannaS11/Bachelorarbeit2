...
if triangle_mesh:
    ...
    for line_start in range(0,np.shape(zyl_points)[0], mini_residual):
        ...
        for i in range(np.shape(distance_min)[0]):
            #x = np.arange(zyl_points[line_start],(1,3))
            #print(f" s{points_min[i].shape} {zyl_points[line_start].shape}")
            ...
            if np.dot(zyl_points[indexes]-zyl_points[line_start], -zyl_normals[line_start]) < 0:
                ...
                if np.dot(zyl_normals[indexes], zyl_normals[line_start]) < 0:
                    ...
                    if distance_min[sort_indices[x]] < 0.2:
                        break
                    midpoints[line_start//mini_residual] = zyl_points[line_start] + (zyl_points[indexes] - zyl_points[line_start]) * 0.5
                    break
