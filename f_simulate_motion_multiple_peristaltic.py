...
def simulate_motion_parallel_2(b_Spline, pcd_colon, min_distances, vector_to_line, t_on_line, speed):
    ...
    locations_ones = np.zeros((int(np.ceil(np.shape(min_distances)[0] / 2)), 2), dtype=int)
    locations_minus_ones = np.zeros((int(np.ceil(np.shape(min_distances)[0] / 2)), 2), dtype=int)
    ...
    for min_dist in range(frequency):
        ...
        t_smallest_distance_arg = np.argwhere(t_on_line == min_distances[min_dist_oral, 0]) 

        # calculations for stopping condition of while loop
        original_point = np.asarray(b_Spline.evaluate_single(min_distances[min_dist_oral, 0]))
