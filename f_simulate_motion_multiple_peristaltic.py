...
def simulate_motion_parallel_2(b_Spline, pcd_colon, min_distances, vector_to_line, t_on_line):
    # anstatt hälfte von minimum anfangen stattdessen bei hälfte von Spline anfangen
    ...
    for min_dist in range(frequency):
        ...
        if np.shape(min_distances)[0] % 2 == 0:
            ...
            zero_ones_array[min_dist_oral] = 1
            zero_ones_array[min_dist_aboral] = 1
        ...
        t_smallest_distance_arg = np.argwhere(t_on_line == min_distances[min_dist_aboral, 0]) 

        original_point = np.asarray(b_Spline.evaluate_single(min_distances[min_dist_aboral, 0]))
        ...
        if start:
            #zero_ones_array = np.fmax(0, zero_ones_array)
            factor = combine_fcts_two_fct(t_on_line, zero_ones_array, list_fct)
            start = False
        else:
            
            zero_ones_array[min_dist_aboral_smaller] = -1
            zero_ones_array[min_dist_oral_smaller] = -1
            factor = combine_fcts_two_fct(t_on_line, zero_ones_array, list_fct)
