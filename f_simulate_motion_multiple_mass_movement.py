...
def simulate_motion_parallel_2(
    b_Spline, pcd_colon, min_distances, vector_to_line, t_on_line, speed, reverse
):
    # initialize window
    ...
    start = True
    divisor = 4
    # for last section of mass movement
    ...
    for min_dist_ in range(frequency):
        ...
        if reverse:
            ...
            t_smallest_distance_arg = np.argwhere(
                t_on_line == min_distances[min_dist, 0]
            )

            original_point = np.asarray(
                b_Spline.evaluate_single(min_distances[min_dist, 0])
            )
