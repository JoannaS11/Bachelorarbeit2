...
def find_distance(start_node, index, mean_thickness):
    global adj_matr
    global pcd_np
    ...
    for i in range(np.shape(pcd_np)[0]):
        ...
        if distance < 0.8 * mean_thickness and i != index:
            adj_matr[index, i] = distance# * (0.3 + distance)
            find_distance(pcd_np[i], i, mean_thickness)
...
def find_line(pcd_np_, mean_thickness): 
    ...
    find_distance(pcd_np[0], 0, mean_thickness) #max_distance_between_points)
