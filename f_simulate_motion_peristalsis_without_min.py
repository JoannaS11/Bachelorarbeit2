...
def simulate_motion_parallel(pcd_colon, vector_to_line, t_on_line):
    ...
    for i in range(500):
        ...
        # p_new = p + 0.7 * d * v
        half_line[:, 0] = points[:, 0] + 0.7 * np.multiply(
            dist_to_z, vector_to_line[:, 0]
        )
        half_line[:, 1] = points[:, 1] + 0.7 * np.multiply(
            dist_to_z, vector_to_line[:, 1]
        )
        half_line[:, 2] = points[:, 2] + 0.7 * np.multiply(
            dist_to_z, vector_to_line[:, 2]
        )
