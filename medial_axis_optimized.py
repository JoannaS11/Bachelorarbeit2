...
def straight_line_between_2_points(point1, point2, no_points=60):
    startpoint = point1
    m = (point2 - point1) / no_points
    line_pc_array = np.ndarray([no_points, 3])
    for x in range(0, no_points):
        line_pc_array[x] = startpoint + m * x
    return line_pc_array


def get_curved_line_between_2_points_cubic(point1, point2, cloud1, cloud2, no_points=200, k1=2.5,k2 = 2.5):
    ...
    tangent1 = ((point1 - dir_point1) * 100) / (np.linalg.norm((point1 - dir_point1)* 100) + 0.001)
    tangent2 = ((point2 - dir_point2) * 100) / (np.linalg.norm((point2 - dir_point2)* 100) + 0.001)

    print(np.linalg.norm(tangent2))
...
def get_curved_line_between_2_points(point1, point2, cloud1, cloud2, no_points=200):
    curved_line = np.ndarray([no_points,3])
    ...
    angle_half = ((point2-dir_point2) + (point1-dir_point1)) / 2
    ...
    print(np.linalg.norm(np.cross(point2-center_2, point1-center_1)))
    if np.linalg.norm(np.cross(point2-dir_point2,  point1-dir_point1))  < 0.01 and np.dot(point2-dir_point1, point1-dir_point1) < 0:
        curved_line = straight_line_between_2_points(point1, point2, no_points=80)
    else:
...
# quadratic bezier between end points
connection_lines = []
start_point = end_points[0][1]
for i in range(len(medial_axis)-1):
    end_point_plus_one, next_start = find_next_end_point(start_point, end_points[i+1])
    connection_lines.append(get_curved_line_between_2_points(start_point, end_point_plus_one, medial_axis[i].contracted_point_cloud, medial_axis[i+1].contracted_point_cloud))
    start_point = next_start

# cubic bezier between end points
connection_lines = []
for i in range(0,len(medial_axis)-1):
    print(f"{i} and i-1 {i+1}")
    if i == 0:
